...
test('home, about and llms.txt all say "vetted" engineers or talent so agents can classify the site', () => {
  for (const file of ['index.html', 'about.html', 'index.md', 'llms.txt'])
    assert.match(read(file), /vetted (AI engineer|Argentinian talent)/i, file)
  assert.doesNotMatch(
